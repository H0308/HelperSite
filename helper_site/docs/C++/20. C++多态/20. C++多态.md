...
!!! note
	...
	~Base()  
	```
	但是多态条件中，父类指针指向子类对象，此时如果使用上面的方法写析构函数，则有可能会出现内存泄漏问题，所以父类析构函数需要设计成虚函数（析构函数会被处	为`destructor()`函数，所以子类和父类同名），这样在父类指针指向一个子类对象时，销毁父类指针会先走子类的析构函数（因为是多态，指针指向哪个对象就调	哪个对象的析构函数），然后再走父类的析构函数，最后释放指针`b`的空间
