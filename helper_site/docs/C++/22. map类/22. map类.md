...
[map官方文档]([map - C++ Reference (cplusplus.com)](https://legacy.cplusplus.com/reference/map/map/?kw=map))
...
[multimap官方文档]([multimap - C++ Reference (cplusplus.com)](https://legacy.cplusplus.com/reference/map/multimap/?kw=multimap))
...
> 构造这个链表的[**深拷贝**](https://baike.baidu.com/item/深拷贝/22785317?fr=aladdin)。 深拷贝应该正好由 `n` 个**全新**节点组成，其中每个新节点的值都设为其对应的原节点的值。新节点的 `next` 指针和 `random` 指针也都应指向复制链表中的新节点，并使原链表和复制链表中的这些指针能够表示相同的链表状态。**复制链表中的指针都不应指向原链表中的节点**。例如，如果原链表中有 `X` 和 `Y` 两个节点，其中 `X.random --> Y` 。那么在复制链表中对应的两个节点 `x` 和 `y` ，同样有 `x.random --> y` 。
> 返回复制链表的头节点。
...
原来的思路是拷贝每一个节点，将拷贝节点链接到对应原节点的后方，再通过拷贝节点和原节点之间的规律进行链接构造一个新的链表，最后将新的链表从原链表中切除下来，示意图如下：

<img src="22. map类.assets/image-20240801222539165.png" alt="image-20240801222539165" />

有了map之后，可以考虑构建原节点和新节点之间的对应关系，即通过原节点（key）找对应的拷贝节点（value），基本思路没有改变
...
public:
    Node *copyRandomList(Node *head)
    {
        ...
        while (cur)
        {
            ...
            {
                // 将拷贝节点的random链接到当前节点的random指向的节点的拷贝节点
                copyTail->random = nodeCopy[cur->random];
            }
