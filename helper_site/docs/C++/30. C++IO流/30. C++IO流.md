...
2. 空格和回车都作为数据之间的默认分隔符，分隔符不会被读入，所以多个数据可以在一行输入，也可以分行输入。但如果是字符型和字符串，则空格（ASCII码为32）无法用`cin`输入，字符串中也不能有空格，对于需要读取带空格的字符串可以使用`cin`对象中的`getline`函数一次获取一行数据并以回车为结束标记
...
上面两种形式，当读取的内容小于用于接收读取内容的容器大小，则会提前结束并设置`eofbit`位。默认情况下，如果`n`大于0，则会自动在存储的字符串后自动添加一个空字符`\0`，即使提取的是空字符串也是如此
...
1. 将数值类型数据格式化为字符串
    ...
    上面的代码中，需要使用`clear`函数，因为`stringstream`在执行一次转换时，会将其内部状态设置为`badbit`，因此下一次转换是必须调用`clear`函数将状态重置为`goodbit`才可以转换，但是`clear`不会将`stringstream`对象底层字符串清空掉，如果不清空，在多次转换时，会将结果全部累积在底层`string`对象中，所以需要使用`void str (const string& s)`方法清空字符串
