...
<img src="images\image2.png" align="left">
...
如果只是定义了 Add 函数，而不调用函数， Add 函数的参数 x和 y 只是形式上存在的，不会向内存申请空间，不会真实存在的，所以称为形式参数。形式参数只有在函数被调用的过程中为了存放实参传递过来的值，才向内存申请空间，这个过程就是形式的实例化。
