...
2. 数组排序：`static void sort(int[] a)`，默认升序。如果比较的是基本数据类型，则可以直接进行比较，如果比较的是引用数据类型，默认情况下该引用类型必须实现`Comparable`接口并提供对应的`compareTo`方法。如果该引用类型没有实现`Comparable`接口，则需要自己实现一个类，该类需要实现`Comparator`接口并提供对应的`compare`方法，在使用`sort`时传递该类的对象即可，这个思路可以类比C++中的[仿函数](https://www.help-doc.top/C%2B%2B/16.%20priority_queue%E6%A8%A1%E6%8B%9F%E5%AE%9E%E7%8E%B0/16.%20priority_queue%E6%A8%A1%E6%8B%9F%E5%AE%9E%E7%8E%B0.html?h=%E4%BB%BF#_1)
