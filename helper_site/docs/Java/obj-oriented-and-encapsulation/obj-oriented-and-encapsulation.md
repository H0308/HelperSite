...
在Java中，封装是将一类事物的属性和方法放到一个`class`类中，例如`Person`类
...
如果一个类中有多个构造函数，例如当类中的无参构造需要调用类中的有参构造时，可以使用`this`进行直接调用：
...
但是需要注意，`this`调用构造函数时必须保证作为构造函数体中的第一行代码
...
Java中的main方法有一个形参`String[] args`，当需要临时测试方法时，可以通过这个形参传递值
