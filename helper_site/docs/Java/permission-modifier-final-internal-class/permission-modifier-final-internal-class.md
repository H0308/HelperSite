...
需要注意的是，如果父类成员是被`protected`修饰，并且子类和父类并不是同包的，那么只有子类对象可以直接访问，其他类对象的类必须与父类同包才可以让该类对象直接访问，例如：
...
!!! note
    ...
    `File`->`Project Structure`->`Modules`->选择需要改变JDK的模块->在`Source`页面改变`Language Level`为`17`->在`Dependencies`页面改变`Module SDK`（SDK即JDK）为`17`->`OK`

    需要修改回JDK8重复上面的步骤，选择JDK8即可

    改变项目JDK版本只需要在选择`Module`的位置选择`Project`即可
