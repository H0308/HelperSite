...
1. `name`：表示Servlet别名
2. `value`和`urlPatterns`：表示Servlet请求映射地址，因为一个Servlet可以配置多个映射地址，所以此处是一个`String`数组。注意，如果设置了`value`值，就不要设置`urlPatterns`的值，二者相互排斥，每一次只会有一个生效。之所以会有value是因为[`value`是存在默认值的，所以使用该注解给`value`赋值时，可以不用写`value`属性名，并且因为`value`被设计成可以不写属性名直接赋值](https://www.help-doc.top/Java/junit-reflect-annotation-enum/junit-reflect-annotation-enum.html#_15)
