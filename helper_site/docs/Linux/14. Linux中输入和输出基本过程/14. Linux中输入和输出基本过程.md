...
但是，在[实现进度条](https://www.helper-wiki.top/Linux/5.%20Linux%E4%B8%8B%E7%9A%84Makefile%E4%B8%8E%E8%BF%9B%E5%BA%A6%E6%9D%A1%E7%A8%8B%E5%BA%8F/5.%20Linux%E4%B8%8B%E7%9A%84Makefile%E4%B8%8E%E8%BF%9B%E5%BA%A6%E6%9D%A1%E7%A8%8B%E5%BA%8F.html#_1)时提到了`\n`可以刷新语言级别的缓冲区，此处为什么用了`\n`也没有刷新，原因就在于语言级别的缓冲区刷新情况有三种：
...
但是这里涉及到一个问题：程序替换时，被替换的代码会受到重定向的影响吗？答案是会的，因为重定向本质改的是`files_struct`结构体，而该结构体存在于进程中，当前这个进程是子进程，程序替换本质只是替换子进程的代码和数据，但是不会替换进程PCB，所以重定向会影响到被替换的代码
...
### 链接基本使用
前面已经通过代码编写好了`mystdio.c`源文件，如果现在需要给其他人使用可以将`mystdio.c`源文件编译成`.o`文件，接着只需要将`mystdio.o`文件和`mystdio.h`文件给别人即可，别人只需要将对应的头文件引入，最后编译时将自己的源文件和`mystdio.o`文件一起编译生成可执行程序即可，具体内容将在[软硬链接部分](#)继续讲解