...
对于`lock`来说，当执行到`movb $0, %al`表示将数字0移动到当前线程的寄存器中，接着执行`xchgb %al, mutex`将互斥锁`mutex`中的值（默认是1）和当前线程寄存器中的值进行交换，此时当前线程的寄存器就是1，`mutex`变量中的值就是0，接着判断当前线程寄存器的值是否是1决定是否可以执行对应的逻辑，否则就需要等待锁。当线程被操作系统唤醒时，就需要执行`goto lock`继续抢锁，重复上面的逻辑。
