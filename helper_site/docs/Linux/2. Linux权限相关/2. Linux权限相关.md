...
从普通用户切换为root用户时，需要注意`su`和`su -`的区别：

1. 当使用`su`命令时，它默认会保留当前用户的环境变量（如`PATH`），只是改变用户ID（`UID`）和组ID（`GID`）。这意味着如果你想要运行一个命令或者执行一些操作，并且需要另一个用户的权限来完成这些任务，你可以使用`su`用户名 来临时切换到那个用户的身份。
2. 使用`su -`或者`su - 用户名`则会直接切换到目标用户。这意味着它不仅改变了`UID`和`GID`，还会加载该用户的`shell`配置文件（如`.bashrc`, `.profile`等），从而提供一个全新的环境。

!!! note
    需要注意：

    1. `su`命令执行的位置在哪个目录下，切换完用户之后就在哪个目录下，但是具体有没有操作权限需要看用户有没有对应的文件操作权限，**建议是切换完用户后使用`cd ~`回到对应用户的家户目录**，如果为了安全，可以使用++ctrl+d++或者`exit`命令退出用户再切换用户
...
在Linux中，每个文件根据不同的用户会有不同的权限，所以Linux查看一个进程是否有操作管道的权限主要是先看进程启动的用户ID（在Linux中称为UID），接着看管道文件指定用户对应的权限是否合适。一个用户的UID在Linux底层也是一串数字，在配置文件中会有对应的映射，如果想要查看指定文件对应的用户UID，可以使用下面的命令：
