...
所以对于这一点，可以考虑设计两个类，一个类是`Listener`，表示的是监听套接字和其对应接口的封装，另一个类是...（待完善）
...
上面已经完成了基本结构，接下来就需要具体实现每个类

首先，对于一个服务器来说，需要监听客户端的连接，所以此处需要创建一个`Listener`类，该类作为`Connection`类的子类，在该类中，因为只有监听的时候才需要创建TCP套接字，所以考虑在`Listener`类中使用前面的`serverSocket`类创建TCP套接字，如下：
...
因为`Listener`类是`Connection`类的子类，所以需要实现`Connection`类中的纯虚函数，目前不提供具体的实现：
...
接下来为了更好得解释如何创建一个基于边缘模式的`epoll`实现完整TCP服务器，下面提供一个基本的使用函数：
