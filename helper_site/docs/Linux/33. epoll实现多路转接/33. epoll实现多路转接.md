...
解决了上面的两个问题后，回到`Listener`类中的`recvData`函数编写剩下的逻辑。注意，要实现边缘触发模式一定要使用`EPOLLET`：
...
void recvData() override
{
	...
	while (true)
	{
		...
		if (ac_socketfd > 0)
		{
			...
			// 2. 通过Connection将链接指针插入到Epoll模型中
...
至此，基于边缘触发模式`epoll`的TCP服务器基本结构完成，现在的问题是还没有处理正常文件描述符的读写，这一点会在下一节具体介绍