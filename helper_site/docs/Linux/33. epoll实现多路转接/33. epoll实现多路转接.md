...
{
private:
	std::unordered_map<int, Connection*> connections_; // 文件描述符与Connection类对象指针的映射关系
};
```
...
```cpp
// 添加文件描述符和关心的事件
void insertFdAndEvents(Connection* con)
{
	auto pos = connections_.find(con->getFd());
	if (pos != connections_.end())
	{
...
void recvData() override
{
	// LOG(LogLevel::DEBUG) << "进入监听套接字的接收函数";
	// 循环接收
	while (true)
	{
		int rerrno = 0;
...
int main()
{
    ...
    // 将Listener套接字构建为Connection对象
    // std::shared_ptr<Connection> con = std::make_shared<Listener>(e_svr);
    Connection *con = new Listener(e_svr);
    // std::shared_ptr<Connection> con = std::make_shared<Listener>();
    con->setFd(con->getFd());
    con->setEvent(EPOLLIN | EPOLLET);
    // 将Connection对象插入到Epoll模型中
    // epollServerModule::EpollServer e_svr;
    e_svr->insertFdAndEvents(con);
    // 启动服务器
    e_svr->startServer();
...
void recvData() override
{
	...
	while (true)
	{
		...
		{
			// ...
			ac_con->setHandleDataFunc(taskModule::Task::task_1);
		}
		// ...
	}
}
...
正常情况下不论多少个客户端连接还是断开，每个客户端都是独立的，不会互相影响