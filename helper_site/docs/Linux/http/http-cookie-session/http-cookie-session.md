...
有了知识基础，接下来就是实现获取时间函数，在前面[命名管道与共享内存](https://www.help-doc.top/Linux/com-process/com-named-pipe-shm/com-named-pipe-shm.html#_8)和[日志系统](https://www.help-doc.top/Linux/linux-thread/log/log.html#_8)已经实现或者使用了一个获取时间的函数，本次实现的思路和该函数基本一致，但是当时使用的是`localtime`或者`localtime_r`函数，这两个函数都会自动包含时区，而实际上在设置`expires`时不需要设置时区，当客户端获取到时间后会自动根据当前浏览器所处的地区设置时区，所以这里推荐使用另外一个系统调用`gmtime`或者其可重入版本`gmtime_r`。下面以`gmtime`为例，函数原型如下：
