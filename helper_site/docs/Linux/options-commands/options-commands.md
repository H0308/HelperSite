...
当前处于`/root/test`目录下，`test1`目录下有一个`test1.c`文件，使用`mv`指令将`test1.c`文件移动到`test`目录下
...
使用`echo`配合追加重定向会在原来内容后的下一行进行写入，**不换行追加可以使用`printf`**
...
在Linux中，可以使用`head`指令取出文件的内容，默认取出前十行内容，也可以通过指定行数`-行数`取出内容。如果要查看动态滚动的日志可以使用下面的命令：
