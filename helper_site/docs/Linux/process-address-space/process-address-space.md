...
前面提到，父进程和子进程会共享代码和数据，尤其是两个进程不进行数据修改时，数据不会产生两份，那么这样理解就可以直观地认为当子进程修改了数据，对应的变量内存地址就会发生改变。但是实际上改变的不是程序读取到的地址，看下面例子的演示结果：
...
需要注意的是，进程地址空间并不是实际存在于物理内存中的一个结构，而是通过虚拟地址由全0到全1整个范围构成的一个逻辑结构
