...
因为操作系统底层不止一个定时器，所以操作系统本身也需要对创建的定时器进行管理，如果直接使用链表，那么就会出现每一次都需要遍历一遍定时器链表找出截止的定时器节点执行其中的操作，再对其进行销毁，此时的时间复杂度就会比较高，所以操作系统会考虑使用更优秀的方法来完成这个动作，例如可以考虑使用小堆结构，**注意操作系统底层并不一定使用堆**。除了使用小堆以外，还可以考虑**时间轮**思想，具体见[仿Muduo库的高并发服务器](javascript:;)
