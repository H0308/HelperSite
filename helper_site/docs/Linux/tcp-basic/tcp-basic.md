...
但是本次不使用上面的任意一种，而是考虑让子进程再创建一个子进程，一旦创建成功就让当前子进程退出，而让新创建的子进程（孙子进程）继续执行后续的代码，因为当前子进程已经退出并且退出前并没有回收新创建的子进程（孙子进程），所以当前孙子进程就会被操作系统托管变成孤儿进程，一旦孙子进程走到了读写逻辑下面的`exit(0)`就会退出，此时操作系统就会自动回收这个孙子进程。这个思路也被称为「双重`fork`（或者[守护进程化](https://www.help-doc.top/Linux/25.%20%E8%BF%9B%E7%A8%8B%E9%97%B4%E5%85%B3%E7%B3%BB%E5%92%8C%E5%AE%88%E6%8A%A4%E8%BF%9B%E7%A8%8B/25.%20%E8%BF%9B%E7%A8%8B%E9%97%B4%E5%85%B3%E7%B3%BB%E5%92%8C%E5%AE%88%E6%8A%A4%E8%BF%9B%E7%A8%8B.html#_6)）」。所以，代码如下：
...
因为所有线程共享一个文件描述符表，所以不需要手动关闭一些文件描述符，下面使用[前面封装的线程](https://www.help-doc.top/Linux/19.%20Linux%E7%BA%BF%E7%A8%8B/2.%20Linux%E7%BA%BF%E7%A8%8B%E5%BA%93%E4%B8%8E%E7%BA%BF%E7%A8%8B%E5%BA%93%E5%B0%81%E8%A3%85/2.%20Linux%E7%BA%BF%E7%A8%8B%E5%BA%93%E4%B8%8E%E7%BA%BF%E7%A8%8B%E5%BA%93%E5%B0%81%E8%A3%85.html?h=%E7%BA%BF#_4)进行演示：
...
classDiagram
    ...
    class TcpServer {
        ...
        +start()
        +stop()
        +read_write_msg(sockaddr_in peer)
        +~TcpServer()
    }
    
    class Command {
        -set<string> _commands
        +Command()
        +isValid(string cmd) bool
        +executeCommand(string cmd) string
        +~Command()
    }
    ...
    class Dictionary {
        ...
        -string _file
        -string _sep
        +Dictionary(string path, string file, string sep)
        +translate(string word) string
        +printDic()
    }
    
    TcpServer --> Command : 使用
    TcpServer *-- ThreadPool : 包含
    ThreadPool o-- Thread : 管理
```
...
sequenceDiagram
    ...
    Server->>+Pool: getInstance()
    Pool-->>-Server: 线程池实例
    Server-->>-Main: 创建成功
    
    Main->>+Server: start()
    Server->>+Pool: startThreads()
    Pool->>Thread: start()多个线程
    Pool-->>-Server: 线程池启动完成
    
    loop 接受客户端连接
        Server->>Server: accept()
        Server->>+Pool: pushTasks(read_write_msg)
        Pool->>Thread: 唤醒线程执行任务
        Thread->>+Server: read_write_msg(peer)
        
        loop 处理客户端命令
            Server->>Server: recv()
            Server->>+Cmd: isValid(buffer)
            Cmd-->>-Server: 返回验证结果
            
            alt 命令有效
                Server->>+Cmd: executeCommand(buffer)
                Cmd-->>-Server: 命令执行结果
                Server->>Server: send(结果)
            else 命令无效
                Server->>Server: send("错误指令")
            end
        end
        
        Server-->>-Thread: 客户端断开连接
    end
```
...
sequenceDiagram
    ...
    loop 命令交互
        Client->>Client: getline(cin, message)
