...
确定了修改位置，接下来思考如何处理添加用户。在前面主题实现类中存在着一个添加用户的方法，根据观察者模式的特点，添加用户本质就属于添加观察者，所以服务器添加用户本质就是调用主题实现类中的添加观察者方法。但是现在的问题是，服务器并不知道如何调用这个方法，所以根据前面实现[翻译软件](https://www.help-doc.top/Linux/udp/udp-app1/udp-app1.html#_7)的思路，可以考虑在创建服务器对象时要求传递对应地添加方法，所以此时就需要一个成员变量用于接收传递的函数，基本结构如下：
