...
2. 解法2：前缀和
    ...
    本题可以考虑使用[同余定理](https://www.help-doc.top/algorithm/other/modulo/modulo.html#_3)将题目转化为求一个前缀和满足`sum % k == tempSum % k`，使用一个哈希表存储余数。剩余的思路与上题思路一致

**关键步骤：**

需要注意，在C++中，`%`对应的运算是取余运算，在计算负数对一个正数取余数时会得到一个负数，此时需要进行[取余运算修正](https://www.help-doc.top/algorithm/other/modulo/modulo.html#_4)。与上题一样，如果存在前缀和为0，而0可以整除任何数，所以需要将前缀和0与对应的次数1建立映射关系，并且也不需要真正创建一个前缀和数组
