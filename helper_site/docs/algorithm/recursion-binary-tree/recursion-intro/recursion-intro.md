...
## 本篇介绍
在[C语言函数](https://www.help-doc.top/c-lang/func/func.html#_18)部分已经简单介绍过递归。递归的本质就是函数自己调用自己，对于其概念和特点已经在该部分提及，本篇主要解决下面个问题：
...
理解一个递归最直观的就是画递归展开图，在[数据结构：二叉树（基础）](https://www.help-doc.top/data-structure/binary-tree/binary-tree.html#_6)部分已经提到过多次递归展开图，此处不再赘述。但是，递归展开图都有一个特点，那就是必须写完代码才能画递归展开图，所以在初学递归时可以使用递归展开图辅助理解递归，在接下来使用递归解题时，就必须想到在脱离根据代码画递归展开图时写出一个递归
