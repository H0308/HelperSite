...
需要注意的是，`this`指针有时可以为空指针，有时不可以为空指针，具体表现为：当对象调用函数**不对数据进行操作（包括访问）**时，`this`指针**可以为空指针**，当对象调用的函数**对数据进行操作（包括访问）**时，`this`指针**不可以为空指针**，否则会出现野指针错误，例如下面的代码：
...
int main()
{
    ...
    p->test2();// p为空指针，调用test2会在获取val处出现野指针错误
