...
7. 析构函数销毁对象时是，其成员变量的销毁顺序是按照其声明的顺序的**相反**顺序销毁的
...
在前面的运算符重载函数中，当运算符重载函数在全局时不可以使用`const`修饰形式参数，因为`const`成员变量传递给成员函数时涉及到引用权限放大，那么`const`成员函数就是用来解决这种权限放大问题，可以将权限保持不变或者缩小，例如在下面的代码中
