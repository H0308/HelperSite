...
在上面的代码中，`less`和`greater`的原型和实现如下：
...
对于`less`和`greater`来说，只要满足对应的条件就返回真，如果为真，对于优先级队列来说就是插入数据，所以对于大堆来说，使用`less<int>`代表当前数值如果小于根节点数值就会因为`less<int>`返回`true`插入当前节点，同理可得`greater`