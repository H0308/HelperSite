...
红黑树是第二种平衡二叉搜索树，为了解决普通二叉搜索树的效率问题，红黑树在控制平衡时选择近似平衡而不是AVL树的绝对平衡，每一个节点都会存储一个表示颜色的属性，包括红色和黑色，通过对颜色的控制，**红黑树可以保证最长路径的长度不会超过最短路径长度的两倍**
...
!!! note
    在上图中，`NIL`表示空叶子节点，以`NIL`作为结束条件，一共有11条路径，需要注意不是以叶子节点为结束条件

满足上面的四个性质，红黑树就可以保证其相对平衡的条件：**红黑树可以保证最长路径的长度不会超过最短路径长度的两倍。**
...
当`cur`节点在`parent`的左子树并且`parent`在`grandfather`的右子树时，此时需要进行右左双旋，将`cur`的颜色更新为黑色，将`grandfather`的颜色更新为红色，过程如下：

<img src="12. 红黑树.assets\image10.png">

当`cur`节点在`parent`的右子树并且`parent`在`grandfather`的左子树时，此时需要进行左右双旋，将`cur`的颜色更新为黑色，将`grandfather`的颜色更新为红色，过程如下：
