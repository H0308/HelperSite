...
??? info "日志轮转和日志回滚"
    ...
    需要注意，**spdlog本身并不支持日志回滚（Log Rollback），但是支持日志轮转（Log Rotation）**。如果想通过spdlog实现日志回滚可以结合C++ 17的filesystem库，关于filesystem具体在[C\+\+ 17相关新特性](https://www.help-doc.top/cpp/new-in-cpp17/new-in-cpp17.html#filesystem)有所介绍，例如：
