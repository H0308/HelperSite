...
如果不想使用格式化字符串，也可以使用`f-string`，即带格式的字符串，使用在需要输出的字符串前写上`f`，在常量字符串中使用`{}`包裹需要输出的变量或者表达式（更多内容见[Python输出与文件操作](https://help-doc.top/python/output-format-fileop/output-format-fileop.html)），例如下面的代码：
