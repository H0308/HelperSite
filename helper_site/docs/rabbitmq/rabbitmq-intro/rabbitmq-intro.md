...
在上面的流程中，虚拟主机实际上是RabbitMQ为不同的消息队列提供的一种逻辑上的隔离机制，类似MySQL的数据库，是一个逻辑上的集合，一个MySQL服务器可以有多个Database。队列就是存储消息的地方，其与消费者属于多对多的关系，即一个消费者可以订阅多个队列，一个队列可以被多个消费者订阅。交换机主要起到路由转发的作用，根据特定类型和规则将消息转发到对应的队列上。具体来说，在RabbitMQ中，交换机有下面四种：

1. Fanout（广播）：将消息转发到所有绑定到当前交换机的队列
2. Direct（定向）：将消息转发到完全符合消息Routing Key（见下文介绍）的队列
3. Topic（通配符）：将消息转发到符合Routing Pattern的队列
4. Headers（请求头）：根据消息内容中的headers属性进行队列匹配

交换机本质不会存储任何消息，只负责消息的转发，如果消息没有落盘在队列上，就会出现消息丢失的问题

Routing Key表示路由键，消息生产者在发送消息时需要指定一个字符串，这个字符串就是交换机用来判断如何处理该消息的方式。类似的，交换机和队列之间的绑定关系可以指定一个Binding Key作为匹配规则，绑定（Binding）用于关联队列和交换机，<a href="javascript:;" class="custom-tooltip" data-title="Binding Key也是Routing Key，只是在用于队列绑定时称其为Binding Key">Binding Key</a>是绑定时指定的匹配规则，具体流程就是交换机根据消息的Routing Key和Binding Key进行比对。在Direct模式中，Routing Key需要与Binding Key完全相同才会走对应的队列；在Topic模式中，Routing Key满足Binding Key的指定规则即可走到对应队列
