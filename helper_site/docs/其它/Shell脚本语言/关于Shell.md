...
`while` 循环检测一个条件，只要这个条件为真，就执行一段命令。被检测的条件跟 `if..then` 中使用的基元并无二异。因此一个 `while` 循环看起来会是这样：
...
!!! note
    ...
    `return` 返回函数返回值，返回值类型只能为整数（0-255）。如果不加 `return` 语句，Shell 默认将以最后一条命令的运行结果，作为函数返回值。
