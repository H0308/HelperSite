...
在开散列时并没有设计析构函数，因为vector会调用自己对应的析构函数，而且vector中的类型也是一个自定义类型会调用对应的析构函数，内置类型不处理，所以可以不用析构函数。但是在闭散列中，每一个哈希数据都是一个在堆上开辟的节点，所以需要额外释放，vector本身的析构函数只能析构`_table`，不能析构每一个节点
