...
有了上面的规律，可以直到矩阵第一列和第二列一共存储的字符个数是$num = 2 \times 总共行数 - 2$个，而因为从上向下，从下斜向上为一个周期，所以对于整个字符串来说，假设字符串元素个数为`n`，一共存在$\lceil \frac{n}{num} \rceil$，而因为每一次周期为`numRows - 1`列，所以得出最后二维矩阵需要的列数`col`为$\lceil \frac{n}{num} \rceil \cdot (numRows - 1)$
...
[力扣54.螺旋矩阵](https://leetcode.cn/problems/spiral-matrix/description/)
...
!!! quote
    ...
    行程长度编码（RLE）是一种字符串压缩方法，其工作原理是通过将连续相同字符（重复两次或更多次）替换为字符重复次数（运行长度）和字符的串联。例如，要压缩字符串`3322251`，我们将`33`用`23`替换，将`222用`32`替换，将`5`用`15`替换并将`1`用`11`替换。因此压缩后字符串变为`23321511`。
