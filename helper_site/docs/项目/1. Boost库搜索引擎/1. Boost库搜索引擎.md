...
!!! note
    ...
    2. 关于JSONCPP的使用可以看[关于JSONCPP库](https://www.help-doc.top/%E5%85%B6%E5%AE%83/%E5%85%B3%E4%BA%8EJSONCPP%E5%BA%93/%E5%85%B3%E4%BA%8EJSONCPP%E5%BA%93.html#jsoncpp)
...
最后，在`BackwardIndexElement`中还有一个字段：权重，本质上这里需要涉及到相关性分析，但是相关性分析是一套很复杂的过程，所以本次为了简便，考虑一种思路：关键字出现在标题的权重值要大于关键字出现在内容的权重值，所以可以设计权重的计算公式为$\text{title_cnt} \times 10 + \text{body_cnt}$，这样就完成了一篇文档内所有关键字的统计
...
完成上面的逻辑之后，最后就是将结果转换为JSON字符串存储到`search`函数的第二个参数中。因为需要给用户返回的内容时文章标题、文章内容和文章在官网的URL，而此处的`BackwardIndexElement`并没有这三个字段，所以此处还需要借助正排索引，这就是为什么需要同时在正排索引文章信息节点`SelectDocInfo`和`BackwardIndexElement`设置文档ID字段的原因。一旦获取到文章信息后，就可以将三个字段的内容转换为JSON字符串并赋值给`json_string`
...
通过网址到官网启动浏览器搜索可以看到结果总共是25，其中标题中也存在`split`，按照设定的权重计算公式$\text{title_cnt} \times 10 + \text{body_cnt}$应该是35而并非34
