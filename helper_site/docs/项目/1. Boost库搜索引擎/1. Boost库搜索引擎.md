...
接下来考虑关键字的问题，关键字既可能出现在`title`中，也可能出现在`content`中，那么必然需要对`title`和`content`两部分进行分词解析提取出其中的关键字，这里可以为每一个关键字建立一个结构体`WordCount`，这个结构体中存储着当前关键字在当前文档中的标题和内容部分出现的次数，所以有两个变量`title_cnt`和`content_cnt`，而为了建立关键字和对应的关键字出现次数结构体之间的映射关系，就需要用到一个`unordered_map<string, WordCount>`，分别统计`title`和`content`中关键字出现的次数更新对应字段的值即可统计出当前关键字在当前文档中出现的次数

最后，在`BackwardIndexElement`中还有一个字段：权重，本质上这里需要涉及到相关性分析，但是相关性分析是一套很复杂的过程，所以本次为了简便，考虑一种思路：关键字出现在标题的权重值要大于关键字出现在内容的权重值，所以可以设计权重的计算公式为$title_cnt \times 10 + content_cnt$，这样就完成了一篇文档内所有关键字的统计

最后，既然是“多对多”的关系，那么除了考虑一个文档内含有多个关键字以外，还需要考虑一个关键字对应多个文档的情况。实际上，因为每一个文档都需要统计当前文档中所有的关键字，而一旦所有的文档全部统计完毕就实现了一个关键字对应多个文档
...
根据`getPath`函数的逻辑：如果用户指定的字典文件路径为空，那么就会读取默认路径的值例如如果`dict_path`为空，`jieba.dict.utf8`就会作为默认文件路径，在函数内部会调用`pathJoin`将父路径、`dict`目录和当前`jieba.dict.utf8`拼接形成一个路径，根据`Jieba.hpp`的文件位置：`include/cppjieba`，也就是说如果要使用原始文件就需要将`cppjieba/dict`目录拷贝到`cppjieba/include`目录中，对于这种情况，使用的目录结构如下：
