...
previewBox.classList.add('active');setTimeout(()=>{iframe.src=url;},50);let loadTimeout=setTimeout(()=>{if(iframe.style.display==='none'){showError(loading);}},15000);iframe.onload=function(){clearTimeout(loadTimeout);setTimeout(()=>{loading.style.display='none';iframe.style.display='block';},100);};iframe.onerror=function(){clearTimeout(loadTimeout);showError(loading);};}
