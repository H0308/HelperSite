...
let isInitialized=false;function initLinkListeners(){if(isInitialized)return;isInitialized=true;document.addEventListener('click',function(e){const link=e.target.closest('a');if(!link)return;const isInPreviewBox=link.closest('.link-preview-box');if(isInPreviewBox){if(!isValidLink(link))return;const url=getFullUrl(link);if(!url)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(previewBox&&previewBox.classList.contains('active')){currentUrl=url;const iframe=previewBox.querySelector('.link-preview-iframe');const loading=previewBox.querySelector('.link-preview-loading');iframe.style.display='none';loading.style.display='flex';loading.className='link-preview-loading';loading.innerHTML='加载中...';iframe.src=url;}
return;}
